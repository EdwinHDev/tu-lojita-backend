  ...
  async create(createDto: CreateStorePaymentMethodDto, storeOwnerId: string) {
    ...
    if (!store) {
      throw new NotFoundException(
        'No se encontró la tienda o no tienes permisos sobre ella',
      );
    }
    ...
    if (bankId) {
      ...
      if (!bank)
        throw new NotFoundException(`Banco con ID ${bankId} no encontrado`);
    }
  ...
  async update(
    ...
    if (paymentMethod.store.owner?.id !== storeOwnerId) {
      throw new ForbiddenException(
        'No tienes permiso para editar este método de pago',
      );
    }
